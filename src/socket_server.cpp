...
void rgbImageCallback0(const sensor_msgs::ImageConstPtr& msg){
    ...
    try
    {
        printf("trans the format to opencv\n");
...
void depthImageCallback0(const sensor_msgs::ImageConstPtr& msg){
    ...
    try
    {
        cvImgPtr = cv_bridge::toCvCopy(msg, sensor_msgs::image_encodings::TYPE_32FC1);//TYPE_32FC1
...
int main(int argc, char **argv){
    ...
    //ofs_off.close();
    // topic: image
    image_transport::Subscriber subdepth = it.subscribe("/image_raw", 1, depthImageCallback0);//camera/depth/image_raw
    image_transport::Subscriber subrgb = it.subscribe("/tango/camera/color_1/image_raw", 1, rgbImageCallback0);//camera/rgb/image_raw
